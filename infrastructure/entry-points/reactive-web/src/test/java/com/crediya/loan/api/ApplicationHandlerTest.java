...
class ApplicationHandlerTest {
    ...
    private GenerateRequestUseCase useCase;

    @BeforeEach
    void setup() {
        useCase = Mockito.mock(GenerateRequestUseCase.class);
        ApplicationMapper mapper = Mappers.getMapper(ApplicationMapper.class);
        Validator validator = Validation.buildDefaultValidatorFactory().getValidator();
        ApplicationHandler handler = new ApplicationHandler(useCase, mapper, validator);
    ...
    void createApplication_ok() {
        Mockito.when(useCase.execute(any(Application.class))).thenAnswer(inv -> {
        ...
        client.post()
                ...
                // LocalDate serializado como arreglo [yyyy, M, d]
